...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://coldrelay.com/use-cases/${page.slug}` },
    openGraph: {
      type: "article",
      url: `https://coldrelay.com/use-cases/${page.slug}`,
...
export default async function UseCasePage({ params }: PageProps) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Use Cases", item: "https://coldrelay.com/use-cases" },
      { "@type": "ListItem", position: 3, name: page.title, item: `https://coldrelay.com/use-cases/${page.slug}` },
    ],
  };
      ...
      <section className="pt-32 sm:pt-40 pb-8 sm:pb-12">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 mb-6">
            <span className="text-xs font-medium tracking-wide uppercase text-cyan-400">
              {page.industry}
            </span>
          </div>
      ...
      <article className="py-10 sm:py-14">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
          ...
          <section>
            ...
            {sections.painPoints.content.split("\n\n").map((para, i) => (
            ...
            <div className="space-y-6">
              {sections.howItWorks.steps.map((step, i) => (
                <div key={i} className="flex gap-4">
                  <div className="flex-shrink-0 w-10 h-10 rounded-full bg-[#4A73D5]/20 text-[#6B8FE6] font-bold text-sm flex items-center justify-center">
                    {i + 1}
                  </div>
                  <div>
              ...
              <table className="w-full text-sm text-left">
                <thead>
                  <tr className="border-b border-white/[0.06] bg-white/[0.03]">
                    <th className="px-4 py-4 font-semibold text-white">Metric</th>
                    <th className="px-4 py-4 font-semibold text-[#6B8FE6]">Benchmark</th>
                    <th className="px-4 py-4 font-semibold text-white/50">Notes</th>
                  </tr>
                </thead>
                <tbody>
                  {sections.metrics.benchmarks.map((row, i) => (
                    <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-4 py-4 text-[#6B8FE6]">{row.value}</td>
                      <td className="px-4 py-4 text-white/50">{row.notes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-2xl border border-[#4A73D5]/20 bg-[#4A73D5]/[0.04] p-8 sm:p-10 text-center">
            ...
            <p className="text-base text-white/50 mb-8 max-w-xl mx-auto">
              Get dedicated domains, mailboxes, and IPs set up in hours — not weeks. Starting at $0.55/mailbox/month.
            </p>
